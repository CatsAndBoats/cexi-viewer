...
`cexi-tools` already parses both, so this script just re-shapes its output:
...
Run (needs the cexi-tools checkout and its venv — same requirement as battle-table.json):

    cd D:\\cexi-tools && uv run python D:\\cexi-gl2\\dev\\gear-models.py
...
from cexi.gear.xi_core import RACE_TABLES, SLOTS, parse_race_table, slot_file_ids
from cexi.ftable.xi_core import load_all_tables, scan_file_ids

OUT = Path(__file__).with_name("gear-models.json")

# Viewer race id -> (cexi race table, look race byte) and, for Tarutaru, the alternate
...
# Viewer slot key -> cexi slot name (the viewer calls the ranged slot "range"). Keyed by
...
def main() -> None:
    ...
    def slots_for(cexi_race: str) -> dict[str, dict[str, int]]:
        """{viewer slot: {DAT: model_id}} for one cexi race table."""
        out: dict[str, dict[str, int]] = {}
        for view_slot, cexi_slot in SLOT_MAP.items():
            mapping: dict[str, int] = {}
            for model_id, file_id in per_race[cexi_race][cexi_slot]:
    ...
    out: dict[str, dict] = {}
    for race_id, (cexi_race, look_race) in RACE_MAP.items():
        entry: dict = {"lookRace": look_race, "slots": slots_for(cexi_race)}
