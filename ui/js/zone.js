// FFXI zone DAT parser — pure JS port of cexi's Python tooling, so the browser
...
//
// Ports: src/cexi/entity/anim/xi_export.py (parse_sections, Reader),
//        src/cexi/zone/xi_decrypt.py        (decrypt_zone_mesh / _objects),
//        src/cexi/zone/xi_export.py         (parse_zone_mesh_section, parse_zone_def),
//        src/cexi/entity/mesh/xi_export.py  (parse_texture, DXT/palette decode).
...
function parseZoneMeshSection(bytes, dv, section) {
  ...
  for (let m = 0; m < meshCount1; m++) {
    ...
    if (isStrip) {
      ...
      // winding stays consistent after the break (cexi xi_export.py).
      let parity = 0;
...
// src/cexi/zone/xi_collision.py decode_collision. Returns flat, non-indexed
...
// Match cexi export + leveleditor (incl. fog/haze/mist celestial shells).
...
export function parseZone(datBuffer, keyTables) {
  ...
  for (const s of sections) if (s.typeCode === 0x1C) {
    ...
    // Filter placements hidden by cexi's delete mechanism (moved to y ≈ -100000).
