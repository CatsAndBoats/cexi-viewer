    ...
    <div className="modal-backdrop" onPointerDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="modal" ref={panelRef} style={style}>
        ...
        <div className="modal-body">
          ...
          <div className="form-row">
            <label className="form-label">cexi-tools CLI (for model export)</label>
            <div className="form-inline">
              <input
                type="text"
                value={draft.cexiPath ?? ''}
                spellCheck={false}
                placeholder="Path to cexi.exe — leave blank to disable model export"
                onChange={(e) => setDraft({ ...draft, cexiPath: e.target.value })}
              />
              <Button onClick={browseCexi}>
                <span className="icon">description</span>
                Browse
              </Button>
            </div>
            <div className="form-hint">Model (glTF/FBX) export shells out to the cexi CLI. Music/SFX WAV export works without it.</div>
          </div>
