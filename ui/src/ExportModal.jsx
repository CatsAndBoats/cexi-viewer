 ...
 * `cexi mesh export` with the chosen options (glTF/FBX, parts, pose, weld, …).
...
export function ExportModal({ open, spec, onClose, onStatus }) {
  ...
  const needsCexi = isModel && !spec.cexiPath;   // model export requires the cexi CLI
  ...
  const doExport = async () => {
    ...
    try {
      if (isModel) {
        ...
        const out = await backend.cexiMeshExport(spec.sourcePath, folder, args, spec.cexiPath);
        onStatus?.(`Exported ${spec.datStem}.${outExt} → ${folder}`);
        console.log('[cexi mesh export]', out);
        onClose();
      } else {
    ...
    <div className="modal-backdrop" onPointerDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="modal" ref={panelRef} style={style}>
        ...
        <div className="modal-body">
          ...
          {needsCexi && (
            <div className="export-warn">
              ...
              <span>Model export needs the <strong>cexi-tools</strong> CLI. Set its path in
        ...
        <div className="modal-actions">
          ...
          <Button className={`active export-go${busy ? ' busy' : ''}`} onClick={doExport} disabled={busy || !folder || needsCexi}>
            {busy
